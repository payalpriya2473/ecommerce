...
// the live server. Both are re-based onto the API URL this admin already talks
// to (NEXT_PUBLIC_API_URL + "/uploads/..."), so an image works wherever the
// API works.
//
...
// Images are fetched through the API path ("<API_URL>/uploads/...", which the
// backend serves as "/api/uploads/..."): the live nginx only forwards "/api/..."
// to the backend, so a bare "/uploads/..." would hit the Next.js app and 404.
export const ASSET_ORIGIN = (process.env.NEXT_PUBLIC_ASSET_URL || API_URL).replace(/\/+$/, "")
