...
export function buildStockSql(filters = {}, { includeOpqty = true } = {}) {
  const whereClause = buildConditions(filters);
  const sql = `select fstk.*,pord.penord from (SELECT
            finalSTK.TSTOCK, finalSTK.DSTOCK, finalSTK.ASTOCK, finalSTK.ITEM, finalSTK.incentive,  finalSTK.offerprice, finalSTK.PDEL, finalSTK.ITEMGROUP, finalSTK.BRAND,
            sch.schamt,finalSTK.hsncode,finalSTK.gst,finalSTK.margin_prc,finalSTK.opqty,finalSTK.minqty,finalSTK.mopprc,finalSTK.mopamt,
            finalSTK.nlc,finalSTK.category
            FROM
            (
                SELECT
                    STK.TSTOCK, STK.DSTOCK, STK.ASTOCK, STK.ITEM, STK.incentive, STK.offerprice,  STK.ITEMGROUP, STK.BRAND, PDEL.PDEL,STK.hsncode,STK.gst,STK.margin_prc,STK.opqty,STK.minqty,STK.mopprc,STK.mopamt,
                ...
                FROM
                    ...
                    SELECT
                        ...
                        ITEMMAST.BRAND,
                        psrno.item,itemmast.hsncode,itemmast.gst,itemmast.width as margin_prc,itemmast.opqty,itemmast.minqty,itemmast.mopprc,itemmast.mopamt,
                        itemmast.show as nlc,itemgroup.category
                    ...
                    GROUP BY
                        ...
                        itemmast.mrp,
                        ITEMMAST.GROUP1,
                        ITEMMAST.BRAND,psrno.item,itemmast.hsncode,itemmast.gst,itemmast.width as margin_prc,itemmast.opqty,itemmast.minqty,itemmast.mopprc,itemmast.mopamt,itemmast.mrp as offerprice,
                        itemmast.show,itemgroup.category
                    ) AS STK
                LEFT JOIN
                (
                    SELECT
                        COUNT(SALES.ITEM) AS PDEL,
                        ITEM
                    FROM
                        SALES
                    WHERE
        ...
        (
            SELECT
                SUM(incamt) AS schamt,
                item
            FROM
                scheme
            WHERE
                sdate <= DATEVALUE(NOW()) AND
                edate >= DATEVALUE(NOW())
            GROUP BY
                item
        ) AS sch
        ...
        order by
        finalSTK.ITEMGROUP,
        finalSTK.BRAND,finalSTK.ITEM) as fstk left join
        (select sum(bqty) as penord,item from sorderdet where bqty > 0 group by item) as pord on pord.item = fstk.item`;


  return includeOpqty ? sql : sql.replace(/(finalSTK|STK|itemmast)\.opqty,/g, '');
}
...
export async function fetchLiveStockRows(companyCode, filters = {}) {
  ...
  try {
    ...
    try {
      const rows = await conn.query(buildStockSql(filters, { includeOpqty: true }));
      return rows.map(normalizeStockRow);
    } catch (err) {
    
      console.warn(`[accessStock] query with opqty failed (${err.message}) — retrying without opqty`);
      const rows = await conn.query(buildStockSql(filters, { includeOpqty: false }));
      return rows.map(normalizeStockRow);
    }
  } catch (err) {
